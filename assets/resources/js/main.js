...
(function (Components) {
    class Carousel {
        ...
        prepend(num) {
            this.$content.prepend($(this.$elements[num]).clone());
        }
        toLeft() {
            if (this.scrolling)
                return;
...
(function (Components) {
    class Filter {
        ...
        constructor(callBeforeSend, callAfterSend, dataOptions) {
            ...
            window.onpopstate = (event) => { this.popStateEvent(event); };
            ...
            if (dataURI.length && data.type1_size)
                this.sizeRadioFirst.setValue(data.type1_size.toString(), false);
            if (dataURI.length && data.type2_size)
                this.sizeRadioSecond.setValue(data.type2_size.toString(), false);
            if (this.analog) {
                this.useAnalog();
            }
            else {
        ...
        prepareSendData(pushState = true) {
            this.callBeforeSend();
            this.sendData = this.getFilterData();
            const uri = Components.URI.toString(this.sendData);
            if (pushState)
...
(function (Components) {
    class Select {
        ...
        constructor($sourceSelect) {
            ...
            /* Building DOM */
            $wrap.append(this.$imgWrapHover.append(this.$imgHover), this.$header, this.$list.append(this.getOptions()));
            /* Events */
